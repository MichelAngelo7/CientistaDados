...
```{r}
plot(jitter(trees$Girth), trees$Volume, ylab = "Circunfêrencia", xlab = "Volume", col = "blue", main = "Árvores")
```
Legenda com dimensão categórica
```{r}
plot(CO2$conc, CO2$uptake, pch=20, col = CO2$Treatment)
legend('topleft', legend=c("nonchilled","chilled"), cex= 1, fill = c("black","red"))
```
Novos dados
```{r}
plot(trees)
